...
const MyInventory = () => {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      axios.get(`/api/vendor/my-products/${session.user.email}`)
        .then(res => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            ...
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.3em] mt-2">Manage your listed assets and stock.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((item) => (
            <div key={item._id} className="group bg-white dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 rounded-[2rem] overflow-hidden flex flex-col hover:border-blue-600/40 transition-all duration-500">
              ...
              <div className="relative h-48 w-full overflow-hidden">
                ...
                <div className="absolute top-4 left-4 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full border border-white/10">
                   <span className="text-[9px] font-black text-white uppercase tracking-widest">{item.category || "General"}</span>
                </div>
              </div>

              {/* Content */}
              <div className="p-5 flex-grow">
                <h3 className="text-lg font-black uppercase italic dark:text-white truncate mb-1">{item.productName}</h3>
                <div className="flex items-center justify-between mb-4">
                   <span className="text-blue-600 font-black text-sm italic">৳{item.price.toLocaleString()}</span>
                   <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-tighter">Qty: {item.stock || 0}</span>
                </div>

                {/* Actions */}
                <div className="flex items-center gap-2 pt-4 border-t border-zinc-50 dark:border-zinc-800">
                  <button className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all">
                    <Edit3 size={12} /> Edit
                  </button>
                  <button 
        ...
        {products.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 border-2 border-dashed border-zinc-100 dark:border-zinc-900 rounded-[3rem]">
            ...
            <p className="font-black uppercase italic text-zinc-400 tracking-widest text-xs">Your vault is empty_</p>
          </div>
        )}
